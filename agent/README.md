...
对用户自愿提供的本地 JSON 对话数据进行结构化分析，帮助用户理解沟通模式、关系动态、人物印象与互动边界。

本工具不负责获取聊天记录；它处理的是用户已经拥有并放入 `talks/` 的本地 JSON 对话材料。核心目标是把已有对话材料结构化成可分析的关系档案。

本项目不鼓励、不支持在未经同意的情况下分析他人隐私数据，也不用于冒充、操控、监视或替代真实关系沟通。
